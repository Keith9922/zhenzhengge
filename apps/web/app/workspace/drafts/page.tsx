...
export default async function DraftsPage({ searchParams }: DraftsPageProps) {
  ...
  const bannerSource = source === "error" || caseSource === "error" ? "error" : "api";
  const bannerNote = [note, caseNote].filter(Boolean).join("；");

  return (
    <section className="space-y-6">
      <DataSourceBanner source={bannerSource} label="草稿列表" note={bannerNote || undefined} />
      ...
      <div className="grid gap-4">
        {items.length ? (
          ...
          <article className="rounded-3xl border border-dashed border-slate-300 bg-white p-6 text-sm leading-7 text-slate-600">
            当前暂无草稿，可在上方选择案件并生成首版文书。
          </article>
        )}
      </div>
    </section>
