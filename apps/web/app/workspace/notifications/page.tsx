...
import { getNotificationChannels, getNotificationLogs } from "@/lib/notifications";

export const dynamic = "force-dynamic";

export default async function NotificationsPage() {
  const [{ items, source, note }, { items: logs, source: logSource, note: logNote }] = await Promise.all([
  ...
  return (
    <section className="space-y-6">
      <DataSourceBanner source={bannerSource} label="接收方式" note={bannerNote || undefined} />
      ...
      <NotificationConsole items={items} logs={logs} />
    </section>
  );
}
